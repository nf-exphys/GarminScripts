...
write.table(filtered_rr_data, file = "./Text Files/01_02_2021_progression_ride_impute.txt", row.names = F, col.names = F) #writes to txt
...
hrv.data <- LoadBeat(fileType = "RR", HRVData = hrv.data, 
                     Recordname = "./Text Files/01_02_2021_progression_ride_impute.txt")
...
step <- 180 #number of seconds per window
...
summary_dfa %>%
#  filter(value < 0.75) %>%
ggplot(data = ., aes(x=HR, y=value)) + geom_line()
