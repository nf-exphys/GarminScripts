...
l <- length(data)
activities <- data.frame(matrix(NA, nrow=l, ncol = length(data[[1]]$sum_data)))
names(activities) <- names(data[[1]]$sum_data)
for (i in 1:l){
  activities[i,] <- add_column(data[[i]]$sum_data)
}

activities <- activities %>%
  dplyr::select(c(8,9,33,36,37,42,45,49,56,61,70,71,73:78,80,82,84,85))

activities$timestamp <- as.POSIXct(activities$timestamp, origin = "1970-01-01")

#somewhat inefficient way to add season to groups of dates
activity1 <- activities %>%
  filter(timestamp > "2018-05-07" & timestamp < "2018-11-04") %>%
  filter((name == "Run") | (name == "Run Indoor")) %>%
  mutate(season = 2018)
activity1 <- activity1[-172,] #removes cooldown after race

activity2 <- activities %>%
  filter(timestamp > "2019-06-01" & timestamp < "2019-11-29") %>%
  filter((name == "Run") | (name == "Run Indoor")) %>%
  mutate(season = 2019)
  
activity3 <- activities %>%
  filter(timestamp > "2020-03-05" & timestamp < "2020-09-02") %>%
  filter((name == "Run") | (name == "Run Indoor")) %>%
  mutate(season = 2020)

activities <- rbind(activity1, activity2, activity3)

racedates <- c("2018-06-23", "2018-09-29", "2018-11-03", 
               "2019-06-02", "2019-08-17", "2019-11-28", 
               "2020-03-15", "2020-05-03", "2020-05-16", "2020-08-03", "2020-09-01")

# load data:
# exercises of marathon runners 
# user profiles of marathon runners

#tab in denotes my version of the code
...
# group activities by runner_id and marathon_day, order by date and then by distance,
  colnames(activities)[17] <- "distance"
  colnames(activities)[18] <- "duration"
activities <- activities %>% group_by(h_user_id,marathon_day) %>% arrange(distance)
  activities <- activities %>% group_by(season) %>% arrange(distance)
...
training <- summarise(activities,runs=n(),total_dist=sum(distance),total_time_min=sum(duration)/60000,mean_vel=mean(velocity),Mdist=last(distance),Mvel=last(velocity))
  training <- summarise(activities, runs = n(), total_dist=sum(distance), total_time_min = sum(TimeMinutes), mean_vel=avg_speed, Mdist=last(distance), Mvel=last(avg_speed))
...
d_bins <- c(1000,0.97*1600,1.03*1600,0.97*3200,1.03*3200,0.97*5000,1.03*5000,
            0.97*10000,1.03*10000,0.97*21097.5,1.03*21097.5,0.97*42195,1.03*42195,100000)

d_labs <- c(0.,1600.,0.,3200.,0.,5000.,0.,10000.,0.,21097.5,0.,42195.,0.)

as.numeric.factor <- function(x) {as.numeric(levels(x))[x]}

activities$bin <- cut(activities$distance,breaks=d_bins,labels=d_labs)
nas <- which(is.na(activities$bin) == T) #tried setting NAs to zero to see if that would fix the mutate error
activities$bin[nas] <- 0
activities <- activities %>% mutate(duration = as.numeric(duration))
activities <- activities %>% mutate(vel_actual=1000*as.numeric.factor(bin)/duration)
activities_bins <- activities %>% 
  group_by(season,bin)  %>% 
  summarise(vel_max=max(avg_speed), date = timestamp, 
            duration = duration, vel_actual = avg_speed)
  
#11/25 note: Instead of having all of this code to pull out races, just manually take each race from the activities
  #Then reproduce activities_bins, which will become races_by_vel
...
activities_bins <- activities_bins %>% filter((bin!=0))
...
races_by_vel <- activities_bins %>% group_by(season)

races_by_vel <- races_by_vel %>% arrange(season,desc(bin)) %>% 
  mutate(cur_max = ave(vel_max, season, FUN=cummax))
#need to add duration to races_by_vel
races_by_vel <- races_by_vel %>% filter(vel_actual == cur_max) %>% dplyr::select(season,bin,vel_actual,duration) %>% mutate(time=duration/1000)
...
model_fit_3 <- unnest(c(model_fit_2$fit %>% purrr::map(broom::glance)))
  #^throws error
